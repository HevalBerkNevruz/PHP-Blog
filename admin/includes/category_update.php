<?php
    ...
    $id=get("id");
    $query=query("select * from category where category_id='$id'");
    $row=row($query);
?>
<article class="module width_full">
    ...
    if($_POST){
        ...
        if(!$category_name){
            ...
            $is_categorylink=query("select * from category where category_link='$category_link' && category_id!='$id'");
            if(mysql_affected_rows()){
                echo "<h4 class='alert_error'><strong>'.stripcslashes($category_name).'</strong> Adında Bir Kategori Zaten Mevcut</h4>";
            }else{
                $update=query("update category set
            ...
            if($update){
                ...
                go(URL."/admin/index.php?do=/categories&id=".$row["category_id"],1);
            }else{
            ...
            <fieldset>
                <select name="home_category">
                    ...
                    while($category_row=row($category_query)){
                        echo '<option';
                        echo $category_row["category_id"] == $category_row["category_id"] ? ' selected ' : null;
